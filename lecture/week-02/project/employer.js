...
module.exports = class Employer {
  ...
  createJobAd(title, description, zipcode, category, jobType, compensationMin, compensationMax, tips) {
    // eslint-disable-next-line max-len
    const job = new Job(title, description, zipcode, category, jobType, compensationMin, compensationMax, tips);
    job.employer = this;
    // save job to db
  ...
  addInterview(application, option1, option2, option3) {
    ...
    const interview = new Interview(application, option1, option2, option3);
  ...
  updateApplicationStatus(interview, status) {
    ...
    if (status === 'accepted') {
      ...
      const applications = Database.load('application.json');

      notAcceptedApplicationIds.forEach((applicationId) => {
        // edit db entry
