...
const save = function (filename, data) {
    ...
    if (fs.existsSync(filename) && fs.readFileSync(filename, 'utf8').length >= 2) {
        ...
        let dbdata = fs.readFileSync(filename, 'utf8')
        let json = JSON.parse(dbdata)
        json.push(data)
            
        fs.writeFileSync(filename, JSON.stringify(json, null, 2))
        
    } 
    // TODO: handle checking if object already exists by id, so it doesn't get added again when running multiple times w/o having to delete json file.
}
...
module.exports = {save, load, updateEntry, updateObject}
